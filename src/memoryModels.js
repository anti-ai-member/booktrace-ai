...
function normalizeTopicItem(item = {}, index = 0) {
  ...
  return {
    ...
    relatedTo: asStringArray(item.relatedTo || item.related),
    evidence,
    occurrences: normalizeOccurrences(item.occurrences, evidence),
    updatedAt: item.updatedAt || new Date().toISOString(),
  };
...
function normalizeArgumentItem(item = {}, index = 0) {
  ...
  return {
    ...
    respondsTo: asStringArray(item.respondsTo),
    evidence,
    occurrences: normalizeOccurrences(item.occurrences, evidence),
    updatedAt: item.updatedAt || new Date().toISOString(),
  };
...
function mergeMemoryItem(left, right) {
  ...
  return {
    ...
    prerequisites: uniqueStrings([...(left.prerequisites || []), ...(right.prerequisites || [])]),
    relatedTo: uniqueStrings([...(left.relatedTo || []), ...(right.relatedTo || [])]),
    supports: uniqueStrings([...(left.supports || []), ...(right.supports || [])]),
    respondsTo: uniqueStrings([...(left.respondsTo || []), ...(right.respondsTo || [])]),
    claim: preferText(right.claim, left.claim),
    reason: preferText(right.reason, left.reason),
    example: preferText(right.example, left.example),
    conclusion: preferText(right.conclusion, left.conclusion),
    evidence,
