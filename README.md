...
- **EPUB / PDF / MOBI 阅读**：本地解析 EPUB、PDF（文本层）、MOBI / AZW / AZW3（非 DRM）；目录跳转、高度感知分页、按书恢复进度；EPUB / MOBI 正文插图保留在阅读流中。
...
| 格式 | 说明 |
|------|------|
| **EPUB** | 完整支持；正文插图保留在阅读流中 |
| **PDF** | 支持带文本层的 PDF；扫描版 / 纯图片 PDF 暂无法提取正文 |
| **MOBI / AZW / AZW3** | 支持非 DRM 的 Kindle 电子书（基于 foliate-js）；带 DRM 的文件无法解析 |

**导入入口已预留、解析尚未接入：** TXT、HTML、RTF、DOC / DOCX、FB2、DjVu、CBZ / CBR 等。

一次可多选文件导入；不支持的格式会跳过并提示。
